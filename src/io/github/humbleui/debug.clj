(ns io.github.humbleui.debug
  ...
  (:import
    ...
    [io.github.humbleui.skija Canvas Font]
...
(defn draw-impl [^Canvas canvas tag]
  (with-open [bg   (paint/fill 0x4033CC33)
    ...
    (let [ms->y #(- height (-> % (* height) (/ max-time-ms)))
      ...
      (canvas/draw-rect canvas (IRect/makeLTRB 0 0 width height) bg)
      (canvas/draw-rect canvas (IRect/makeLTRB 0 (ms->y 16) width (+ (ms->y 16) 1)) bg)
      ...
      (doseq [i (range width)
        ...
        (canvas/draw-rect canvas (IRect/makeLTRB i (ms->y ms) (+ i 1) height) fg)))))
